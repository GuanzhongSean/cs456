...
class RawSocketListener():
    ...
    def __init__(self, handler, middlebox_port, iface_name):
        ...
        self.listener = sock.socket(
            sock.AF_PACKET, sock.SOCK_RAW, sock.ntohs(0x0003))

    def listen_forever(self):
        try:
            blacklist = open("blacklist.txt", "r").read().strip().split(',')
        except OSError as e:
            print(f"Could not access blacklist.txt: {e.strerror}")
            exit(1)
        ...
        while True:
            ...
            self.handler(message, self.middlebox_port,
                         self.iface_name, blacklist)
...
def main():
    ...
    the_server = RawSocketListener(
        l2_handler, args.middlebox_port, args.iface_name)
